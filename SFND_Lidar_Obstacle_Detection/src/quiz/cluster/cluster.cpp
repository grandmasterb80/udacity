...
int main ()
{
    ...
    for(int cluster = 0; cluster < 100; cluster++ )
    {
        ...
        {
            float px = cx - 0.5f *cloudsize + cloudsize * rand() / INT_MAX;
            float py = cy - 0.5f *cloudsize + cloudsize * rand() / INT_MAX;
            float pz = cz - 0.5f *cloudsize + cloudsize * rand() / INT_MAX;
            std::vector<float> newPoint = { px, py, pz };
    ...
    for (int i=0; i<points.size(); i++) 
  	...
  	//
  	std::vector<std::vector<int>> clusters = euclideanCluster(points, tree, 2.0);
	...
	std::vector<Color> colors = { Color(1,0,0), Color(0,1,0), Color(0,0,1), Color(1,1,0), Color(0,1,1), Color(1,0,1),
                                  Color(1,b,b), Color(b,1,b), Color(b,b,1), Color(1,1,b), Color(b,1,1), Color(1,b,1)
    };
