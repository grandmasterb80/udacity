...
for i = 1:(Ns-T-G)
    ...
    noiseLevel = sum(s(i:i+T-1)); % + sum(s(i+T+2*G+1:i+N-1));
    %for j = 0:(T-1)
    %    noiseLevel = noiseLevel + s(i + j) + s(i + j + T + 2*G + 1)
    %end
    %threshold = ( sumTrainingCells / (2 * T) ) * offset
    threshold = ( noiseLevel / ( 1 * T ) ) * offset
